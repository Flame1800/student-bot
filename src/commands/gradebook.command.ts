...
export class GradebookCommand extends Command {
    ...
    handle(): void {
        ...
        this.bot.action(/period:(.*)$/, async (ctx) => {
            ...
            const currPeriod: Period | undefined = ctx.session.periods.find((period: Period) => periodId === period.id)
            ...
            try {
                if (ctx.session.user && currPeriod) {
                    ...
                    const creditsResponce = await getCredits(currPeriod.id, ctx.session.user.user_id)
        ...
        this.bot.action(/credit:(.*)$/, async (ctx) => {
            ...
            const credit = ctx.session.credits.find((credit: Credit) => credit.id === creditId)
            ctx.session.credits = []
